...
const ThemeSwitcher = () => {
	...
	localStorage.setItem('theme', newTheme); // Sace to localStorage
	};

	return (
        <div className='flex gap-2'>
            <button className={`flex gap-2 p-2 ${theme === 'dark' ? 'bg-[color:var(--container-color)] shadow-sm' : 'hover:bg-[color:var(--container-color)] hover:shadow-sm'} rounded-md text-sm items-center`} onClick={() => toggleTheme('dark')}>
            ...
            <button className={`flex gap-2 p-2 ${theme === 'light' ? 'bg-[color:var(--container-color)] shadow-sm' : 'hover:bg-[color:var(--container-color)] hover:shadow-sm'} rounded-md text-sm items-center`} onClick={() => toggleTheme('light')}>
                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
